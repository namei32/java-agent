...
| R6 | 渠道与控制面 | R6.1–R6.4 已合入；R6.5 G0 已批准 | 版本化 Message Runtime、本地 CLI、Provider Streaming、Telegram 与可靠投递已离线验收；Loopback 控制面从 G1 实施 |
...
状态：R6.1–R6.4 已实现、验证并合入 `main`；R6.4 合并后 CI 稳定性修复也已通过 PR #8 和主分支三套门禁；R6.5 G0 已批准并从 G1 实施，真实 Telegram Smoke 待授权，R6 整体仍在进行中。
...
- R6.5 已批准默认关闭的 Loopback Bearer Session、安全状态、未来事件 SSE 与目标取消 Contract，并从 G1 实施；只管理 Servlet 模式中当前存活的 Telegram Turn，不把 CLI、同步 HTTP 或 `EXECUTION_UNKNOWN` 伪装成可控对象。
