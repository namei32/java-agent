...
状态：R12-S1 的只读 Skill Catalog Contract、ADR、设计和连续 TDD 计划已冻结；尚未实现 Kernel Port、Workspace
Adapter、Prompt 接线或 Properties。该切片可以与 R11 的无执行安全基础并行，但不绕过 R11 的副作用 Capability
门禁。
