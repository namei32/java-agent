...
状态：R12-S1 的只读 Skill Catalog 已实现并验证：Kernel Port、Workspace Adapter、Prompt 接线和严格
Properties 均已落地，13 Case Fixture 以及 `clean verify`、`-Pfailure verify`、`-Pcompat verify` 均通过。该
切片可以与 R11 的无执行安全基础并行，但不绕过 R11 的副作用 Capability 门禁。
