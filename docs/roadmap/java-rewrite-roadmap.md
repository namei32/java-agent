...
| R13 | 多渠道、Dashboard 与控制面 | C0–C1、C2-A、C2-B 已完成；C3-C0 已冻结，C3-C1–C5 未开始 | 20 Case Fixture 已由默认关闭的本机 `GET /api/v1/control/index` 消费；22 Case C2-A Fixture 已由 `GET /api/v1/control/history` 消费；31 Case C2-B Fixture 已由零正文 `GET /api/v1/control/history/detail` 消费。C2-B 仅投影当前 Scope 的 role/time，默认 Scope Resolver/Port 拒绝，SQLite 仅在临时 Java 数据库/Fake 验证。C3-C0 仅选择既有 Scope 受限 `forget_memory` 的受审批执行契约，不增加 Route、Worker、真实数据或渠道。现有能力均在既有 Loopback/Bearer 后工作；没有用户/Python/生产历史读取、IPC、QQ、Feishu、Dashboard、前端、真实渠道或 CLI+Web；它们继续冻结或待单独批准 |
...
- Keyword/RRF、Query Rewrite/HyDE、自动对话提取、Consolidation 与 Optimizer 实现继续冻结；Optimizer 只冻结 Java DB Revision、事务和 Undo 边界。
- R12-S5 的[当前 Scope 只读记忆召回 Tool](../contracts/read-only-memory-recall-tool.md)已作为受限替代完成：默认关闭、Deferred、
  仅 Java Native 当前 Scope、无 DML。它不授权 Python 记忆迁移、写入、跨 Scope、真实 Provider 或部署启用。
...
## R12：Skills、MCP 扩展与 Plugin 生命周期

状态：R12-S1 的只读 Skill Catalog、S2 的 MCP Resources/Prompts 元数据目录、S3 的 API v2 只读
Plugin Lifecycle Tap、S4 的 deferred `read_skill` 和 S5 的当前 Scope `recall_memory` 均已实现并验证；各切片的 Fixture、`clean verify`、
`-Pfailure verify`、`-Pcompat verify` 均通过。它们可以与 R11 的无执行安全基础并行，但不绕过 R11 的副作用
Capability 门禁。
...
`tool_search` 按名回送已审计、可用且无 frontmatter 的正文，不暴露路径或执行 Skill。ADR-0029 已确认 Python Skill
只是模型指令资产，不存在需迁移的 SkillRunner；正文中要求的动作仍由各 Tool Capability 逐项授权。Streamable HTTP 和
可变 Plugin 生命周期仍属于后续 R12 切片，均需新的 Fixture 与 Contract。R12-S5 的 `recall_memory` 只在
`JAVA_NATIVE`、全局 `READ_ONLY`、`CURRENT_SCOPE_READ_ONLY` 同时启用时，经当前 Turn 搜索后出现；它复用既有
cosine/Hotness 稳定排序，但无 Python evidence、Keyword/RRF、时间线或写入权限。

退出门禁：每个 R12 子切片的 Fixture、Disabled 零 I/O、失败路径、Prompt/生命周期纵向测试和三套阶段门禁
通过；真实网络、Python import 与任何由 Skill 文本建议的可执行能力均保持独立授权。
