...
| R14 | 主动、自动记忆与 Peer | P0–P5 已完成并验证（仅离线 Fake/静态 Catalog） | P3 已为单个 `NOTE` 捕获完成独立 Approval/Anchor/AES-GCM Capsule 与 Fake Memory Recovery；P4 固定唯一 `LOCAL_FAKE` Manifest/Card、1,024 code points/5 秒/单并发预算、Fake Port 与取消恢复；P5 仅有两个 `DEFERRED` 空参数 Placeholder Schema。均无 Bootstrap、SQLite Adapter、Memory DML、网络、进程、真实投递或自动运行；自动提取/Optimizer、真实 Peer/A2A 与生产执行仍未开始 |
