...
| R12 | Skills、MCP 扩展与 Plugin 生命周期 | S1–S3 已实现并验证 | 默认关闭的只读 Skill Catalog、MCP Resources/Prompts 元数据目录、预算、失败隔离与 Stale，以及 API v2 精确只读 Lifecycle Tap；按需正文、远程 MCP 与可变生命周期仍未开始 |
...
状态：B1 Tool Catalog 与 B2a Local Approval Inbox Foundation 已实现并验证；B2b Pending Operation Contract 的无执行状态机、AES-GCM 参数胶囊、v2 原子 Store、同库 `CONSUMED`/唯一 `RESERVED`、并发单获胜者、Ledger 终态、Session 条件提交、初始 Session Anchor 原子写入、安全 Result 的 Anchor 条件提交、不可恢复 Cursor 拒绝、测试专用 Fake Capability 零重放演练与 54 场景 Fixture 已完成；B3 已实现默认关闭的独立 Root `read_file`/`list_dir`（严格路径/链接、UTF-8、预算、Unicode code-point 排序、Deferred Schema 与失败隔离）。完整 `clean verify`、`-Pfailure verify`、`-Pcompat verify` 均通过。Resume/Cancel/Status 的 24 场景 Loopback Message Contract 已冻结但零路由；生产恢复编排与真实副作用 Capability 尚未实现；R11 尚未完成。
...
状态：R12-S1 的只读 Skill Catalog、S2 的 MCP Resources/Prompts 元数据目录，以及 S3 的 API v2 只读
Plugin Lifecycle Tap 均已实现并验证；各切片的 Fixture、`clean verify`、`-Pfailure verify`、`-Pcompat verify`
均通过。它们可以与 R11 的无执行安全基础并行，但不绕过 R11 的副作用 Capability 门禁。
...
API v1 Wire 不变、默认零 I/O，外部 stdio 条目必须显式 `api-version: 2`。完整按需 Skill、正文读取/注入、
Streamable HTTP 和可变 Plugin 生命周期仍属于后续 R12 切片，均需新的 Fixture 与 Contract。
