...
| R6 | 渠道与控制面 | R6.1、R6.2 已合入；R6.3 离线已验证；R6.4 草案待审批 | 版本化 Message Runtime、本地 CLI、Provider Streaming 与 Telegram Channel 已离线验收；可靠投递决策材料已形成，真实 Smoke 与 Dashboard 仍待后续切片 |
...
- R6.4 已形成独立 Java SQLite 渠道账本、Inbox/Claim、事务 Outbox、`UNKNOWN` 与回退草案，尚未获批或进入生产实现。
...
- R6.4 持久 Inbox/Outbox 与恢复草案已形成并等待批准；不预先承诺自动重放或 Exactly Once。
