...
| R10 | Prompt 编排、Persona 与 Provider 协议 | Prompt/Persona 与 Provider P0 已实现 | Java-owned Section/Fixture、context frame、注入时间/会话、固定预算裁剪、默认 `MINIMAL` 与 `AKASHIC_CORE` 接线均通过；Provider P0 已实现脱敏稳定失败分类，Options/thinking/cache/裁剪恢复（P1–P4）仍未开始 |
