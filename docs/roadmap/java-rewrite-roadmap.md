...
| R2 | 被动聊天纵向切片 | MVP 已完成，能力对齐未完成 | HTTP 非流式聊天、SQLite、模型适配、失败与并发语义 |
...
- 补齐领域不变量、历史选择边界、并发等待、Schema 幂等/约束、事务回滚、成功日志和健康失败测试。
- 清理编译与测试告警；若第三方限制无法消除，记录原因和升级条件。
...
退出门禁：本轮 Minor 清单关闭；默认、`failure`、`compat` 全绿；真实模型 Smoke 由人工提供凭证后执行；R0 的被动聊天 Golden 基线完成。
