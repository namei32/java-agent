...
| Provider 适配策略与 thinking/cache 细节 | `agent/provider.py`、`bootstrap/providers.py` | 一个 OpenAI-compatible Spring AI 适配器；Tool/流/超时/取消已有本地验收，P0 已增加安全拒绝/上下文超限的脱敏稳定分类，P3 已验证默认关闭、Tool 前非流式的本地上下文超限恢复，P4 已验证只包含三项整数的匿名缓存用量聚合 | [R10 Provider 协议计划](../plans/2026-07-19-r10-provider-protocol-alignment-plan.md)的 P3/P4 已通过三套完整门禁；P1 Options 已审计但等待运行语义选择，P2 reasoning 仍未开始。P4 不等同于 Python Dashboard 或持久诊断。真实 Provider Smoke 仍独立批准 |
