...
| Dashboard 会话/消息/记忆管理与前端 | `bootstrap/dashboard_api.py`、`frontend/` | 后端 Loopback 状态/取消、审批 Inbox、R13-C1 活动索引及 C2-A 默认关闭的内存终态目录；零前端 | C1 的 20 Case Fixture 投影活动 Turn/渠道健康；C2-A 的 22 Case Fixture 只投影短期 opaque ref/channel/终态/完成时间。它们均不读取 Session/Message/Memory/SQLite 正文，不开启远程/CLI+Web/前端，也不提供详情或写入。C2-B/C3–C5 仍待单独批准 |
