...
| Dashboard 会话/消息/记忆管理与前端 | `bootstrap/dashboard_api.py`、`frontend/` | 后端 Loopback 状态/取消、审批 Inbox，及 R13-C1 默认关闭的本机只读索引；零前端 | C1 已将 20 Case Fixture 落为认证后的活动 Turn/渠道健康最小投影与一次性 opaque cursor；它不读取历史、不开启远程/CLI+Web/前端，也不提供 Session/Message 浏览或写入。C2–C5 仍待单独批准 |
