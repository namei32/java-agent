...
| 模型调用 | `agent/provider.py` | `adapter-spring-ai` | 部分 | OpenAI-compatible 同步文本、Tool Call 和 SSE 文本流已完成；Provider Options/模型配置保留，空闲超时、损坏流和定向传输取消有本地 HTTP 验收。R10-P0 已迁移安全/上下文失败的脱敏、不可重试稳定分类；厂商 Options、thinking/Tool continuation、cache usage 与裁剪重试仍未迁移。分层边界见 [R10 Provider 计划](../plans/2026-07-19-r10-provider-protocol-alignment-plan.md)，真实外部流式 Smoke 仍需授权 | 低 |
