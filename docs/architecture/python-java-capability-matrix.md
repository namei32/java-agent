...
| 模型调用 | `agent/provider.py` | `adapter-spring-ai` | 部分 | OpenAI-compatible 同步文本、Tool Call 和 SSE 文本流已完成；Provider Options/模型配置保留，空闲超时、损坏流和定向传输取消有本地 HTTP 验收。R10-P0 已迁移安全/上下文失败的脱敏、不可重试稳定分类；P1 已提供默认关闭的显式 DeepSeek/DashScope 文本 thinking/effort allowlist，任一 Tool Schema 强制抑制；P3 已通过默认、`failure`、`compat` 门禁，提供默认关闭的 `SAFE_LOCAL` Tool 前非流式上下文恢复；P4 已从标准 prompt/cache-read 数字投影匿名 Turn 聚合，并只在提交后通知默认 no-op Port。完整 thinking/Tool continuation（P2）仍未迁移；P1 不做 URL 推断或任意 body 转发，P4 不保留 native Usage、内容、身份或 Python Dashboard。分层边界见 [R10 Provider 计划](../plans/2026-07-19-r10-provider-protocol-alignment-plan.md)，真实外部流式 Smoke 仍需授权 | 低 |
...
| 能力 | Python 基准位置 | Java 位置 | 状态 | 主要差距/下一步 | 数据风险 |
| --- | --- | --- | --- | --- | --- |
| Tool 协议与注册 | `agent/tools/base.py`、`registry.py` | `agent-kernel`、`ToolRegistry`、`ToolCatalog`、`ApprovalInbox` | 部分 | Approval/副作用/幂等协议、整批门禁和生产 Deny All Framework 已实现；R11 B1 提供常驻/Deferred Catalog、Turn-scoped `tool_search`、确定性 CJK/精确检索和 Schema 逐轮投放；B3/B4 提供 Workspace 与当前会话只读 Tool；R12-S5 添加只读、当前 Scope、无写入的 `recall_memory`。Resume/Cancel/Status Message Contract 已冻结但零路由；仍缺生产 Ledger 恢复编排与具体副作用 Tool Contract。 |
...
| Dashboard/控制面 API | `bootstrap/dashboard_api.py` | R6.5 后端已完成 Fixture、Registry、Event Hub、Telegram 接入、默认关闭 Spring 装配、Loopback Session、安全状态/取消 API、future-only SSE、安全审计和共享关闭 Deadline | 部分 | 已通过 PR #9 合入 `main`；[R13 计划](../plans/2026-07-19-r13-dashboard-channel-alignment-plan.md)已冻结只读 API、受审批写入、前端和渠道的分离路径。前端、远程访问、CLI+Web、同步 Chat、历史/删除/Proactive/Plugin 不在当前范围 | 中 |
...
2. R10 已完成并通过三套阶段门禁；R11 的 B1 Tool Catalog、B2a Local Approval Inbox、B2b Pending Operation 的无执行状态机、B3 默认关闭的独立 Root `read_file`/`list_dir`，以及 B4 当前 Session 只读 `fetch_messages`/`search_messages` 均已完成并通过三套门禁。B4 提供 opaque ID、显式 Scope、SQLite 当前 Session 限定、deferred Schema 和预算。Resume/Cancel/Status Message Contract 仍冻结但零路由。R12-S1 只读 Skill Catalog、S3 API v2 只读 Lifecycle Tap 和 S4 deferred `read_skill` 已分别通过完整三套门禁；ADR-0029 已确认 Python Skill 是指令资产而非待迁移的执行器，S2 MCP Assets 目录是 Java-owned 扩展，不改变 Python MCP 对齐进度。下一实际主线是先取得首个副作用 Capability 的逐工具授权，再完成生产恢复编排；候选与明确边界见[选择记录](../plans/2026-07-19-r11-first-side-effect-capability-selection.md)。
