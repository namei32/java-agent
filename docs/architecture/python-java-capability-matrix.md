...
| Proactive | `agent/core/proactive_*`、`bootstrap/proactive.py` | R8 Proactive Gate/NoOp Planner；R14-P0/P1，P2 本地候选、Fake Pending/Approval/Anchor/Capsule 与 Fake Recovery | 部分 | 默认 NoOp Delivery；P0 固定状态/租约与 `FIXED_LOCAL` Source，P1 完成无正文 skip/pending/cancel 投影，P2 只在同包内存中以一次 claim 将有效候选转为独立 Pending，并以单次 Reservation 调用 Fake Port。全链未接线，没有 SQLite、Outbox、网络、真实渠道或自动 Memory；外部源、真实推送、自动 Memory 与网络仍按 [R14 计划](../plans/2026-07-19-r14-proactive-peer-memory-automation-plan.md)逐项推进 | 极高 |
