...
| Java 原生语义记忆 | Python `memory2/` 仅作历史参考，旧数据不迁移 | `agent-kernel` Memory/Embedding 类型与 Port | 部分 | J1 Fixture 与 J2 纯 JDK 协议已完成；下一步实现 Java Schema 与 Float32 Codec | 高 |
...
1. 按 TDD 完成 R4.2 Task J3 Schema 与 Float32 Vector Codec；生产继续保持 `AGENT_MEMORY_MODE=DISABLED`，`JAVA_NATIVE` 在 J10 前 Fail Closed。
