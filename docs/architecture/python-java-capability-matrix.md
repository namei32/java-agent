...
| Telegram 等渠道 | 渠道模块与 Bootstrap | `agent-bootstrap/.../channel`、`telegram` | 部分 | 私聊数值 Allowlist、Long Polling、进程内去重、定向取消、终态合并与网络关闭已离线验证；R6.4 可靠投递已批准并实施中，真实 Smoke 待授权 | 高 |
...
| 跨语言/Java Contract Fixture | 已建立 Python/Java Golden；R4.2、R5.1、R6.1、R6.2 与 R6.3 另由生产 Java 实现消费 Java-owned Memory、MCP、版本化渠道消息、Provider Streaming/CLI 和 Telegram Fixture，不运行 Python | 后续随新能力增加 Optimizer、可靠投递和控制面 Contract |
...
5. R6.4 持久 Inbox/Delivery、幂等与恢复 Contract/Spec/ADR/计划已获批准，从 Fixture RED 开始连续 TDD；R5.2 远程 MCP、插件和主动能力仍按独立 Contract 推进，不把 R6.3/R6.4 离线授权扩张为真实渠道或副作用授权。
