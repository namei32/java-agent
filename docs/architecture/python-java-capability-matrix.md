...
| Java 原生语义记忆 | Python `memory2/` 仅作历史参考，旧数据不迁移 | Kernel Memory 协议、`JdbcJavaMemoryStore`、`SpringAiEmbeddingAdapter`、Memory Application/HTTP、Bootstrap、R12-S5 `recall_memory` | 完成 | J1 至 J12 已完成独立 V1 库、显式 Write/List/Delete、Embedding、检索与默认关闭装配；R12-S5 再增加仅当前 Chat Scope 的 Deferred 只读召回替代。R14-P6 已冻结临时 SQLite 的获批 `NOTE` 写入演练 Contract，但尚未实现，也不授权生产自动写回/Optimizer、Python 数据迁移或跨 Scope | 高 |
...
| Proactive | `agent/core/proactive_*`、`bootstrap/proactive.py` | R8 Proactive Gate/NoOp Planner；R14-P0–P5 本地 Candidate、逐 Mutation Fake Memory Pending/Recovery、静态 Fake Peer Pending/Recovery 与 Deferred Catalog | 部分 | 默认 NoOp/未接线。P3 仅让 `FIXED_LOCAL` 候选在独立 Approval/Capsule/Anchor 后调用 Fake Memory Port；P4 仅让静态 Card 在同样边界后调用 Fake Peer Port；P5 只是可搜索的空参数 Placeholder。P6 仅冻结临时 SQLite `NOTE` 写入演练，尚未实现；没有生产 SQLite Adapter、Outbox、网络、真实渠道、真实进程、自动 Memory 或 Worker；外部源、真实推送和自动优化仍按 [R14 计划](../plans/2026-07-19-r14-proactive-peer-memory-automation-plan.md)逐项推进 | 极高 |
