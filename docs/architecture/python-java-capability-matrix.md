...
| 能力 | Python 基准位置 | Java 位置 | 状态 | 主要差距/下一步 | 数据风险 |
| --- | --- | --- | --- | --- | --- |
| Tool 协议与注册 | `agent/tools/base.py`、`registry.py` | `agent-kernel`、`ToolRegistry`、`ToolCatalog`、`ApprovalInbox` | 部分 | Approval/副作用/幂等协议、整批门禁和生产 Deny All Framework 已实现；R11 B1 提供常驻/Deferred Catalog、Turn-scoped `tool_search`、确定性 CJK/精确检索和 Schema 逐轮投放；B2a 提供默认关闭的本地 SQLite Inbox、Loopback 单次决定和无执行边界，B2b 已验证 AES-GCM 胶囊及 v2 原子 Store。缺 Pending Turn 消费/恢复、Durable Side Effect Ledger 与具体 Tool Contract | 中 |
...
2. R10 已完成并通过三套阶段门禁；R11 的 B1 Tool Catalog 与 B2a Local Approval Inbox 已完成聚焦验证，B2b Pending Turn 的无执行状态机、AES-GCM 参数胶囊及 v2 原子 Store 已完成聚焦验证，随后是条件提交、Reservation/Ledger 消费和逐工具 Capability Contract。
