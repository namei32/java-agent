...
| Dashboard/控制面 API | `bootstrap/dashboard_api.py` | R6.5 后端能力加 R13-C1 默认关闭的 `GET /api/v1/control/index`：既有 Loopback Session/安全过滤后，只投影活动 Turn、渠道健康、20/50 分页与一次性 opaque cursor | 部分 | C0 的 20 Case 已由 C1 消费；无历史 Session/Message/Memory 浏览、删除、优化、前端、远程访问、CLI+Web、同步 Chat 或真实渠道。C2–C5 仍须逐项 Contract 与批准 | 中 |
