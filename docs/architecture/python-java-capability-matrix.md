...
| Java 原生语义记忆 | Python `memory2/` 仅作历史参考，旧数据不迁移 | Kernel Memory 协议、`JdbcJavaMemoryStore`、`SpringAiEmbeddingAdapter`、Memory Application/HTTP、Bootstrap、R12-S5 `recall_memory` | 完成 | J1 至 J12 已完成独立 V1 库、显式 Write/List/Delete、Embedding、检索与默认关闭装配；R12-S5 再增加仅当前 Chat Scope 的 Deferred 只读召回替代。R14-P6 已实现临时 SQLite 的获批 `NOTE` 写入演练，不授权生产自动写回/Optimizer、Python 数据迁移或跨 Scope | 高 |
...
下表直接维护 Python Tool 与 Java Capability 的名称、语义差异、Runtime/Tool 区分和依赖顺序；
...
| Dashboard/控制面 API | `bootstrap/dashboard_api.py` | R6.5 后端能力，加 R13-C1 默认关闭的 `GET /api/v1/control/index`、C2-A 内存终态目录，以及 C2-B 默认拒绝的 `GET /api/v1/control/history/detail` | 部分 | C0 的 20 Case、C2-A 的 22 Case 与 C2-B 的 31 Case 均有 Fixture。C2-B 只投影当前 Scope 的 `USER`/`ASSISTANT` 时间和 role；默认 Scope Resolver 与 Snapshot Port 均拒绝，SQLite 仅用临时 Java 数据库/Fake 验证。C3-C0/F1–F3/M0 只冻结并验证既有 Scope 受限 `forget_memory` 的受审批执行边界；M0 明确不增加 Dashboard 创建 Route、Memory 目标 Ref 或真实数据权限。Session/Message/Memory 浏览、其他删除/优化、前端、远程访问、CLI+Web、同步 Chat 或真实渠道仍须逐项 Contract 与批准 | 中 |
...
| Proactive | `agent/core/proactive_*`、`bootstrap/proactive.py` | R8 Proactive Gate/NoOp Planner；R14-P0–P5 本地 Candidate、逐 Mutation Fake Memory Pending/Recovery、静态 Fake Peer Pending/Recovery 与 Deferred Catalog；P6 临时 SQLite NOTE 演练 | 部分 | 默认 NoOp/未接线。P3 仅让 `FIXED_LOCAL` 候选在独立 Approval/Capsule/Anchor 后调用 Fake Memory Port；P4 仅让静态 Card 在同样边界后调用 Fake Peer Port；P5 只是可搜索的空参数 Placeholder。P6 仅让同类候选经独立 Approval/Capsule/Anchor 在测试 `@TempDir` 调用 Java SQLite，验证 Ledger 重放/强化与失败闭环；没有生产 SQLite Adapter、Outbox、网络、真实渠道、真实进程、自动 Memory 或 Worker；外部源、真实推送和自动优化须按对应 Capability Contract 逐项推进 | 极高 |
...
| 生产切换/退役 | Python 部署与数据目录 | R9 sandbox Cutover Contract、`SandboxCutoverAdapter`、离线 CLI/Runbook | 部分 | 已完成仅新建 sandbox 的备份、Manifest、差异、验证和回退检查点；配置、实例、数据副本、灰度与退役门槛仍受 [生产切换 Contract](../contracts/production-cutover.md) 约束。真实数据、双写、部署、停止 Python 与退役仍需独立人工授权 | 极高 |
| Peer Agent | `agent/peer_agent/` | R14-P0 值对象；P4 静态 `LOCAL_FAKE` Manifest/Card/预算、Fake Port Pending/Recovery | 部分 | P4 只接受唯一 `peer-local-fake` / `local-fake-peer-v1`，不包含命令、工作目录、环境、PID、网络或 A2A。Fake 验证已覆盖取消、并发、输出净化、失败与零重放；真实进程、远端信任和 A2A 仍未开始 | 极高 |
