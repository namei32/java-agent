...
| 模型调用 | `agent/provider.py` | `adapter-spring-ai` | 部分 | OpenAI-compatible 同步文本、Tool Call 和 SSE 文本流已完成；Provider Options/模型配置保留，空闲超时、损坏流和定向传输取消有本地 HTTP 验收。R10-P0 已迁移安全/上下文失败的脱敏、不可重试稳定分类；P3 已通过默认、`failure`、`compat` 门禁，提供默认关闭的 `SAFE_LOCAL` Tool 前非流式上下文恢复；P4 已从标准 prompt/cache-read 数字投影匿名 Turn 聚合，并只在提交后通知默认 no-op Port。厂商 Options 与 thinking/Tool continuation 仍未迁移；P4 不保留 native Usage、内容、身份或 Python Dashboard。分层边界见 [R10 Provider 计划](../plans/2026-07-19-r10-provider-protocol-alignment-plan.md)，真实外部流式 Smoke 仍需授权 | 低 |
