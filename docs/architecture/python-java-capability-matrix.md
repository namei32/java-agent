...
| Java 原生语义记忆 | Python `memory2/` 仅作历史参考，旧数据不迁移 | `agent-kernel` 协议、`adapter-sqlite` V1 Schema/Codec/Store | 部分 | J1 至 J4 已完成；下一步实现 Spring AI Embedding Adapter | 高 |
...
1. 按 TDD 完成 R4.2 Task J5 Spring AI Embedding Adapter；生产继续保持 `AGENT_MEMORY_MODE=DISABLED`，`JAVA_NATIVE` 在 J10 前 Fail Closed。
