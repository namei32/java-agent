...
| 能力 | Python 基准位置 | Java 位置 | 状态 | 主要差距/下一步 | 数据风险 |
| --- | --- | --- | --- | --- | --- |
| Tool 协议与注册 | `agent/tools/base.py`、`registry.py` | `agent-kernel`、`ToolRegistry`、`ToolCatalog`、`ApprovalInbox` | 部分 | Approval/副作用/幂等协议、整批门禁和生产 Deny All Framework 已实现；R11 B1 提供常驻/Deferred Catalog、Turn-scoped `tool_search`、确定性 CJK/精确检索和 Schema 逐轮投放；B3/B4 提供 Workspace 与当前会话只读 Tool；R12-S5 添加只读、当前 Scope、无写入的 `recall_memory`。B2c 已实现默认关闭的 Scope 软失效 Recovery/Resume/Cancel/Status，但没有 Tool/Chat 生产器；待按 [Pending 生产器提案](../plans/2026-07-19-r11-memory-forget-tool-chat-pending-producer-plan.md)单独冻结和授权该 Contract。 |
...
2. R10 已完成并通过三套阶段门禁；R11 的 B1 Tool Catalog、B2a Local Approval Inbox、B2b Pending Operation 的无执行状态机、B3 默认关闭的独立 Root `read_file`/`list_dir`、B4 当前 Session 只读 `fetch_messages`/`search_messages`，以及 B2c 默认关闭的 Scope 软失效 Capability/Resume/Cancel/Status 已完成。B2c 不注册 Tool/Chat、没有 Worker 或真实执行。R12-S1 只读 Skill Catalog、S3 API v2 只读 Lifecycle Tap 和 S4 deferred `read_skill` 已分别通过完整三套门禁；ADR-0029 已确认 Python Skill 是指令资产而非待迁移的执行器，S2 MCP Assets 目录是 Java-owned 扩展，不改变 Python MCP 对齐进度。下一实际主线是按 [B2c Pending 生产器提案](../plans/2026-07-19-r11-memory-forget-tool-chat-pending-producer-plan.md)冻结并单独授权模型入口；候选语义边界见[选择记录](../plans/2026-07-19-r11-first-side-effect-capability-selection.md)。
