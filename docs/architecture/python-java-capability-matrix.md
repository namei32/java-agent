...
| 能力 | Python 基准位置 | Java 位置 | 状态 | 主要差距/下一步 | 数据风险 |
| --- | --- | --- | --- | --- | --- |
| Tool 协议与注册 | `agent/tools/base.py`、`registry.py` | `agent-kernel`、`ToolRegistry`、`ToolCatalog`、`ApprovalInbox` | 部分 | Approval/副作用/幂等协议、整批门禁和生产 Deny All Framework 已实现；R11 B1 提供常驻/Deferred Catalog、Turn-scoped `tool_search`、确定性 CJK/精确检索和 Schema 逐轮投放；B3/B4 提供 Workspace 与当前会话只读 Tool；R12-S5 添加只读、当前 Scope、无写入的 `recall_memory`。B2c 已实现默认关闭的 Scope 软失效 Recovery/Resume/Cancel/Status，但没有 Tool/Chat 生产器；仍缺该 Pending 投影 Contract 与其他副作用 Tool Contract。 |
