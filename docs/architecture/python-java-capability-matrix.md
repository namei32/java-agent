...
| 启动与装配 | `bootstrap/app.py`、`bootstrap/wiring.py` | `agent-bootstrap` | 部分 | 已装配配置兼容、HTTP 被动聊天、只读 Tool Loop 和默认关闭的静态 MCP Runtime；R6.1 Message Runtime 尚未装配 CLI/渠道宿主，后台能力待迁移 | 低 |
| 入站 HTTP | Dashboard/Bootstrap API | `ChatController` | 完成 | 当前只支持同步 JSON API | 低 |
| 消息总线 | `bus/` | `agent-kernel/.../channel`、`MessageTurnService`、`BoundedOutboundBuffer` | 部分 | R6.1 已完成版本化入站/出站、严格序号、唯一终态、取消、安全错误和有界背压；缺 CLI/渠道 Adapter、真实 Delta 来源和运行时宿主 | 中 |
...
| 失败语义 | `agent/core/runtime_support.py`、错误上下文 | `ToolLoop`、`SafeChatUseCase`、`MessageTurnService`、HTTP 异常映射 | 部分 | 模型、只读工具、迭代上限、提交和 R6.1 渠道终态已映射为稳定安全错误；缺真实渠道 Adapter 与副作用工具错误契约 | 低 |
| 会话内并发 | Python Chat Lane/队列 | `KeyedSessionExecutionGate`、`TurnCancellationSource`、`BoundedOutboundBuffer` | 部分 | 单 JVM 同会话串行、取消原因 First Writer Wins 和有界渠道背压已完成；缺真实 HTTP/CLI 断连接线与跨进程租约 | 中 |
...
| 能力 | Python 基准位置 | Java 位置 | 状态 | 主要差距/下一步 | 数据风险 |
| --- | --- | --- | --- | --- | --- |
| CLI/渠道宿主 | `bootstrap/channel_host.py`、`bootstrap/channels.py` | 无 | 未开始 | R6.1 Contract Runtime 已就绪；R6.2 先接本地 CLI，再考虑真实渠道 | 中 |
| Telegram 等渠道 | 渠道模块与 Bootstrap | 无 | 未开始 | 需消息 ID、重试、乱序和幂等契约 | 高 |
| 流式输出 | Bus/Channel 生命周期事件 | Kernel Channel Contract、`BoundedOutboundBuffer`、`MessageTurnService` | 部分 | Delta/完成/取消/失败顺序、唯一终态、断开和背压语义已固定；当前只投影非流式完成快照，缺真实 Provider Delta 与 Adapter 渲染 | 中 |
...
| 跨语言/Java Contract Fixture | 已建立 Python/Java Golden；R4.2、R5.1 与 R6.1 另由生产 Java 实现消费 Java-owned Memory、MCP 和版本化渠道消息 Fixture，不运行 Python | 后续随新能力增加 Optimizer 与真实 Provider Streaming/渠道 Adapter Contract |
