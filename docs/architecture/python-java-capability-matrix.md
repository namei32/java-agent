...
| Proactive | `agent/core/proactive_*`、`bootstrap/proactive.py` | R8 Proactive Gate/NoOp Planner；R14-P0 Fixture/Kernel 边界、P1 未接线 ReadOnly Decision Runner、P2-A 本地候选准备器 | 部分 | 默认 NoOp Delivery；P0 固定状态/租约与 `FIXED_LOCAL` Source，P1 完成无正文 skip/pending/cancel 投影，P2-A 在同步内存中重新执行 Gate/Fake Source/ReadOnly Drift 并仅保留脱敏候选。三者均未接线，且没有 Approval、Pending、Outbox、网络或自动 Memory。外部源、真实推送、审批、自动 Memory 与网络仍按 [R14 计划](../plans/2026-07-19-r14-proactive-peer-memory-automation-plan.md)逐项推进 | 极高 |
