...
Python-reference 夹具包含输入、Python 基准投影、显式规范化规则和 Java 实际输出；Java-owned 夹具则固定已批准 Contract 的输入与预期结果。时间、UUID、模型自由文本等非确定值要注入或最小化规范化，不能用删除所有字段的方式让测试失去意义。R6.5 的候选 `control-plane/loopback-control-plane-v1.json` 只有在 G0 获批后才创建，不能让待批准文档提前进入 Manifest。
...
2. 先评审并批准 R6.5 G0 的默认关闭 Loopback Session、安全状态、Fetch SSE、活动 Telegram Turn 与 First Writer Wins 取消 Contract；批准前不写生产代码；
3. 后端从 48 Case Java-owned Fixture 开始连续 TDD；CLI 因 Non-Web 契约、同步 `/api/v1/chat` 因无 Message Runtime、`EXECUTION_UNKNOWN` 因无存活执行，均不进入 V1 可控对象；
4. 后端合入后另写前端 Contract，决定 React/Vite 资产归属和同源托管；真实 Telegram Smoke、R5.2 远程 MCP、真实副作用和主动能力继续等待各自独立授权。
