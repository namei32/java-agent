...
1. 提交 R6.3 离线实现，完成 Draft PR、默认/`failure`/`compat` 远程 CI 与 Review；
2. 真实 Telegram Smoke 仅在独立任务批准专用 Bot、测试会话、网络、无敏感内容和 Token 撤销后执行，不阻塞离线 PR；
3. 下一功能切片先冻结 R6.4 的 Inbox/Delivery Schema、幂等键、`UNKNOWN`、恢复、备份与回退 Contract；
4. Contract 批准前不增加自动重放、持久 Inbox/Outbox、新数据库表或消息中间件；
