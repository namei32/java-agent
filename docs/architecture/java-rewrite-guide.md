...
- `agent-testkit`：跨语言夹具、Golden 和 Fake。
...
单元测试证明 Java 内部逻辑正确，Golden Test 证明跨语言观察结果一致。建议建立版本化夹具：
...
Golden 更新必须说明是批准的契约变化还是缺陷修复，不能通过批量重录隐藏回归。
...
1. 完成本轮 MVP Minor 加固；
2. 为被动聊天补跨语言 Contract/Golden；
3. 设计并批准核心消息、生命周期和 Tool 协议；
4. 实现最小 Tool Loop；
5. 按 Roadmap 依次迁移 Memory、MCP、渠道和后台能力。
