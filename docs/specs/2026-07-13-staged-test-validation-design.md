...
- Minor 记录在当前任务的“未解决事项”中，由阶段审查或最终整分支审查统一裁决。
- 只有出现真实失败或疑似不稳定时才允许重复运行聚焦测试；重复次数和原因必须在任务提交或 PR 中解释。
...
同时执行当前功能规定的架构依赖、Secret、Workspace、健康检查、日志脱敏和端到端验收命令。
...
- 每个阶段在提交或 PR 中记录验证命令、测试数、失败数、Profile 选择结果和未解决事项，不创建独立执行台账。
- 测试数按互斥集合分别记录；2026-07-17 前的累计 Profile 数保留为历史证据，不回写也不与新口径直接比较。
- 当前任务和 Git 提交是恢复执行位置的依据；跨功能状态以 Roadmap 为准，对话摘要不能替代仓库内状态。
...
- 被动聊天 MVP 的后续任务：删除重复的每任务全量命令，在 SQLite、装配和最终验收三个阶段点执行集中门禁。
