...
- 状态：部分实现；无执行状态机、AES-GCM Capsule 与 SQLite v2 原子 Store 已验证
- 日期：2026-07-18
...
原样迁移，读取先认证 AAD、再重建完整绑定。下一切片才会 CAS 消费 Approval 并建立 Ledger Reservation；随后才给
`sessions.db` 增加条件 `appendTurnIfNextSequence`；两库不假装拥有全局事务。
