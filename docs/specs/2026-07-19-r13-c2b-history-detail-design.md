...
| schema/role/编码/存储失败 | 脱敏的 `200 DEGRADED CONTROL_SNAPSHOT_UNAVAILABLE` 与空 items |
| 运行时已关闭 | 脱敏的 `200 SHUTTING_DOWN CONTROL_SHUTTING_DOWN` 与空 items，不消费 Ref/Cursor、不读取数据 |

## 5. 验收

31 Case compat Fixture 已冻结上述形状和秘密排除；Kernel、临时 SQLite、Servlet wiring、Ref/Cursor 竞争、过期、撤销、
关闭和审计 sink 失败均有 RED/GREEN。C2-B 只运行受影响模块和 Fixture；R13 的三套完整门禁仍在 C3 或 C5 阶段点执行。
