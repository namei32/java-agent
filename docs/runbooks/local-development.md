...
Java-owned Memory、MCP、两个 Message Bus、Telegram、可靠投递与 Loopback 控制面共 7 个 Fixture 不由 Python 生成器维护，只能在对应 Java Contract 获得新批准后人工更新并同步 Manifest。其他 Python 基准夹具只有在对应 Python 行为或已批准 Contract 变化时才重新生成：
...
当前 Python 生成器只重建其管理的 10 个夹具和人工错误夹具的 Manifest 部分，不拥有 7 个 `java-contract` 夹具。运行后必须审查 `manifest.json`，保留全部 Java-owned 条目并按实际文件重算 SHA-256；最终 Manifest 应有 18 个条目。不得仅因生成器重写 Manifest 而删除 Java-owned Contract。
