...
`AGENT_PROACTIVE_INSPECTION_MODE` 默认 `DISABLED`。只有全局 `AGENT_TOOL_MODE=READ_ONLY`、已有
`AGENT_PROACTIVE_MODE=LOCAL_SQLITE` Runtime 已成功启动，且该值显式为 `ACTIVE_RUNTIME` 时，才注册 Deferred
`list_local_proactive_jobs`。它只返回活动 Job 的安全状态摘要；不会自行创建 `proactive-runtime.db`、启动 Scheduler、
读取 Python Scheduler/Workspace，或提供计划创建、取消、投递与审批。
...
Java-owned Prompt 编排、Memory、MCP、Plugin、Proactive、Cutover、两个 Message Bus、Telegram、可靠投递、Loopback 控制面、Tool Catalog、Approval Inbox、Pending Operation 与 Pending Recovery Fixture 不由 Python 生成器维护，只能在对应 Java Contract 获得新批准后人工更新并同步 Manifest。其他 Python 基准夹具只有在对应 Python 行为或已批准 Contract 变化时才重新生成：
...
当前 Python 生成器只重建其管理的 Python/迁移夹具，不拥有 Java-owned 或人工维护的错误夹具。运行后必须审查 `manifest.json`，保留全部 Java-owned 与错误夹具条目并按实际文件重算 SHA-256；2026-07-19 的当前 Manifest 有 43 个条目。不得仅因生成器重写 Manifest 而删除 Java-owned Contract。
