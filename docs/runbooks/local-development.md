...
Java-owned Prompt 编排、Memory、MCP、Plugin、Proactive、Cutover、两个 Message Bus、Telegram、可靠投递、Loopback 控制面、Tool Catalog 与 Approval Inbox 共 13 个 Fixture 不由 Python 生成器维护，只能在对应 Java Contract 获得新批准后人工更新并同步 Manifest。其他 Python 基准夹具只有在对应 Python 行为或已批准 Contract 变化时才重新生成：
...
当前 Python 生成器只重建其管理的 10 个 Python/迁移夹具，不拥有 13 个 `java-contract` 夹具，也不拥有人工维护的错误夹具。运行后必须审查 `manifest.json`，保留全部 Java-owned 与错误夹具条目并按实际文件重算 SHA-256；当前 Manifest 应有 24 个条目。不得仅因生成器重写 Manifest 而删除 Java-owned Contract。
