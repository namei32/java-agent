...
- 阶段：R13-C0
- 状态：已冻结 Contract/Fixture；未创建运行时 API
...
R13-C0 只冻结一个**未来** Loopback 只读索引的消息形状，供 C1 实现有界渠道健康与活跃 Turn 投影时使用。当前不新增
Controller、Route、Bean、数据库查询、前端、SSE、历史读取或写入；现有 `/api/v1/control/*` 表面不改变。
...
20 个 Fixture Case 固定激活、认证、公开字段、排序、分页、输入拒绝与敏感字段禁令。C0 的 `compat` consumer 只验证
Contract Fixture，不能被说成运行时 API。C1 如获继续实现授权，必须让生产 Projection 逐 Case 消费该 Fixture，并补充
Loopback Filter/关闭/容量/失败路径测试；C2 之后才可能讨论受限历史浏览。
