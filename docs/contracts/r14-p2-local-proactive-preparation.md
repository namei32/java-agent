...
## 2. P2-B：Fake Delivery Preparation（后续）

P2-B 只能消费 P2-A 当前调用内产生的候选，并把它转化为一条独立的 `proactive_prepare_delivery` Pending Operation。
该 Capability 的投递目标只能是测试用 opaque Fake Recipient Ref；参数必须进入认证加密 Capsule，外部响应只返回
安全 `operationRef` 与状态。它不能创建 Outbox、Receipt、Channel Client 或实际消息。

P2-B 必须复用 R11 的 Approval、Capsule、Reservation、Ledger、幂等和审计模型；不能以 P1 的
`PENDING_APPROVAL` 投影或新的轻量审批对象替代它。

## 3. P2-C：Fake Recovery 与失败闭环（后续）
...
必须验证：未批准、取消、过期、Anchor/Capsule 不匹配、并发 Reservation、审计/提交失败和关闭均为零 Fake Invoker；
`UNKNOWN` 与 `COMMIT_UNREPORTED` 均不得重放。P2-C 不为 Scheduler、Bootstrap、控制面或真实渠道添加接线。
