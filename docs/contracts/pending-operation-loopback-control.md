...
- 状态：已冻结且由 R11-B2c F4–F6 实现；默认仍无路由，只有获批 Capability 的全部前置显式开启时才映射本机路由，且没有 Worker
- 契约版本：1
- 日期：2026-07-19
- 阶段：R11 B2b / O6 A6
...
> Worker 或 Tool Capability。该实现不注册 `forget_memory` Tool、Chat 生产器或自动 Resume，因此显式配置本身也
> 不会创建待执行操作。
...
兼容性测试逐 Case 绑定生产边界或聚焦测试所有者；它仍不替代 HTTP、SQLite 事务和竞争的聚焦测试。
