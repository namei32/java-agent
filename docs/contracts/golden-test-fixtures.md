...
`tools/runtime-safety.json` 固定 Tool 模式、预算、Schema、Arguments/Result 边界、超时、并发许可与取消场景。`tools/approval-side-effects.json` 使用 Python 生产 Registry/Hook helper 固定风险标签和前置拒绝投影，并以 `migration-contract` 固定 Java 的整批零执行、审批绑定、一次性消费、幂等重放、`UNKNOWN` 停机、生命周期和模式可见性。`tools/tool-catalog-v1.json` 是 Java-owned Fixture，固定常驻/延迟可见性、CJK/精确检索、同批隔离、下一请求 Schema、模式关闭和零执行。`tools/approval-inbox-v1.json` 是 Java-owned 的 29 场景 Fixture，固定独立 SQLite、随机引用、一次性决定、到期/容量、原始绑定标识哈希化、默认关闭、Loopback 投影和“决定不执行工具”的 B2a 边界。`tools/pending-operation-v1.json` 是 Java-owned 的 41 场景 Fixture：除 Pending 状态、AES-256-GCM、篡改、迁移与原子写入外，固定 Approval `CONSUMED` + Operation `CONSUMING` + 唯一 Reservation 的同库原子性、到期、并发/重放失败关闭、无执行 Ledger 终态、Session Cursor 条件追加及纯 Session Anchor Model；它不授予恢复或 Tool 执行权。Java 必须把这些 Case 交给生产 Registry/Coordinator/Loop、Inbox/SQLite/Control 或 Capsule 组件执行；测试 Fake 只能提供确定性 Model、Invoker 与本地 Key Provider，不能替代被测控制流。
