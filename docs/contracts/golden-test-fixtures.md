...
Manifest 当前登记 36 个业务夹具。普通业务夹具固定：
...
`control-plane/pending-recovery-control-v1.json` 是 Java-owned 的 24 场景设计 Fixture：固定默认/仅 Loopback 时均不映射、严格无 Body/Query 的不透明 `operationRef`、未来 Resume/Cancel 的零执行条件、`UNKNOWN`/`COMMIT_UNREPORTED` 的停机，以及 Status 的稳定脱敏投影。它不创建 Controller、错误枚举、Worker、Token 或 Capability；兼容性测试只校验 Fixture 的形状与敏感字段禁令。
...
5. 为生成器管理的 Python 参考夹具生成校验和；人工维护的迁移契约和 25 个 `java-contract` 夹具必须继续保留在最终 Manifest 中。

当前最终 Manifest 共登记 36 个夹具。Python 生成器不拥有 Java Memory、MCP、Provider 失败分类、两个 Message Bus、Telegram、可靠投递、Loopback 控制面、Plugin、Proactive、Cutover、Prompt 编排、Tool Catalog、Approval Inbox 和 Pending Operation 等 Java-owned Fixture；重新生成后，维护者必须保留这些条目并按实际内容同步 SHA-256，再运行 `compat`。禁止因生成器输出而静默移除 Java-owned Contract。
