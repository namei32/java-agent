...
Manifest 当前登记 24 个业务夹具。普通业务夹具固定：
...
5. 为生成器管理的 10 个 Python/迁移夹具生成校验和；人工维护的错误夹具和 13 个 `java-contract` 夹具必须继续保留在最终 Manifest 中。

当前最终 Manifest 共登记 24 个夹具。Python 生成器不拥有 Java Memory、MCP、两个 Message Bus、Telegram、可靠投递、Loopback 控制面、Plugin、Proactive、Cutover、Prompt 编排、Tool Catalog 和 Approval Inbox 等 Java-owned Fixture；重新生成后，维护者必须保留这些条目并按实际内容同步 SHA-256，再运行 `compat`。禁止因生成器输出而静默移除 Java-owned Contract。
