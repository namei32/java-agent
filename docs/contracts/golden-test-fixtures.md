...
Manifest 当前登记 18 个业务夹具。普通业务夹具固定：
...
当前最终 Manifest 共登记 18 个夹具。Python 生成器不拥有 Java Memory、MCP、两个 Message Bus、Telegram、可靠投递和 Loopback 控制面共 7 个 Java-owned Fixture；重新生成后，维护者必须保留这些条目并按实际内容同步 SHA-256，再运行 `compat`。禁止因生成器输出而静默移除 Java-owned Contract。
