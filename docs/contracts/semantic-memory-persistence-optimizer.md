...
- 自动从每轮对话提取事实或偏好。
- 除 R11-B2c 已单独批准且默认关闭的 Scope 受限 `forget_memory` 外的模型可调用 Memory Tool。
...
第 2 步是只读的重放探测：允许读取 Mutation Ledger 和当前 Item 快照，但不得开启写事务或改变任何 Store 数据。因而首次请求的 Embedding 故障可以有一次只读探测，同时仍须保持零数据库 Mutation；同一成功请求的重试则必须在该探测处直接返回，产生零额外 Embedding 费用。
...
同一 Scope 下的同一 `request_id` 重试必须返回 Ledger 中记录的原 Mutation Status 和 Item ID，不能重复 Embedding 或再次强化；Memory 字段投影使用该 Item 的当前快照，不承诺保存历史响应副本。事务失败必须完整回滚条目和 Mutation Ledger。

若该 Item 后续已被显式物理删除，旧 UPSERT 的重试无法在不恢复已删除正文的前提下重建成功响应，因此必须稳定 Fail Closed，且不得重新 Embedding、重建条目或改写 Ledger。DELETE 重试仍只依赖不含正文的 Ledger，继续返回原 `DELETED` 或 `NOT_FOUND`。
...
- R4.2 显式 HTTP `DELETE` 采用物理删除，确保正文和 Embedding 不再存在；Mutation Ledger 只保留不含正文的操作结果。
- R11-B2c 的模型 `forget_memory` 是独立的、获批的 Scope 受限软失效 Capability，不复用或改变本节 HTTP
  删除语义；其状态、批量幂等、检索过滤、再激活和安全结果由
  [Scope 受限 Memory Forget 契约](approved-scope-bound-memory-forget.md) 单独定义。
