...
- 状态：已批准
- 实施状态：Framework 已实现并通过全部阶段门禁（2026-07-14）
- 契约版本：1
- 批准日期：2026-07-14
...
> 本次批准只授权实现默认拒绝的审批与幂等框架、测试工具和安全边界接口；不授权文件写入、Shell、Web 写入、消息发送、记忆修改或其他外部系统变更。每个真实副作用工具仍需独立 Tool Capability Contract。
...
| `issuedAt` / `expiresAt` | 使用注入 Clock；默认有效期为 5 分钟 |
...
版本 1 不实现跨请求挂起和恢复。生产 `ApprovalPort` 必须默认拒绝；真实人类审批渠道必须在后续独立 HTTP/Channel Contract 中解决身份认证、Pending Turn 持久化和恢复语义。
...
版本 1 不授权 Shell。未来 Contract 至少必须：
...
版本 1 不授权 Web 写入或消息发送。未来 Contract 至少必须：
...
当前同步 Chat HTTP 没有身份认证，也不能安全持有请求等待数分钟。版本 1 不通过延长 HTTP Timeout 模拟审批。
...
2026-07-14 已明确批准以下全部边界：

1. R3.2 只实现默认拒绝框架，不实现真实副作用工具。
2. 新增四类 Lifecycle Event，并同步把核心 Tool Contract 升级为版本 2。
3. 新增 `APPROVAL_REQUIRED` 模式，保持兼容默认 `READ_ONLY`、部署显式 `DISABLED`。
4. 混合批次任一未批准时整批零执行。
5. Side Effect 成功但后续 Turn 失败时 Conversation 不提交，Ledger/Audit 保留。
6. Durable Ledger 与真实 Approval Channel 延后，生产始终 Deny All。
