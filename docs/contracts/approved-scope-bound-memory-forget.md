...
当前实现不注册该 Capability 到 Catalog，也没有 Chat 生产器；因此模型不可见、不可创建 Pending。下列条件是已经实现的
Recovery/控制面装配前置，也是后续模型入口提案的必要条件，而不是当前的 Catalog 可见性声明：
...
Pending Operation、零 Approval、零 Invoker 调用和零路由；即使全部条件满足，当前也仍为零 Catalog/Chat 创建面。
Memory/Inbox 自身按其既有、独立配置保持行为。本契约只复用既有本机 Pending Operation 控制路径；不创建
Dashboard、CLI、前端、远程监听或后台自动恢复。
