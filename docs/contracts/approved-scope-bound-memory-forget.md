...
`DISABLED` 是该新属性、模板和测试外运行的唯一默认值。任一条件不满足时为零 Catalog 注册、零 SQLite
打开、零 Pending Operation、零 Approval、零 Invoker 调用和零路由。本契约只复用既有本机 Pending
Operation 控制路径；不创建 Dashboard、CLI、前端、远程监听或后台自动恢复。
