# R12-S5 当前 Scope 只读记忆召回 Tool 契约（提议）

- 阶段：R12-S5
- 状态：提议，未获实现授权；默认 `DISABLED`
- 契约版本：1（草案）
- 日期：2026-07-19
...
> 本文只冻结可审阅的安全替代方案，**不授权生产代码、Tool 注册、Embedding 调用、真实 Provider、真实用户数据或
> 部署启用**。R4.2 已批准范围仍明确排除模型可调用 Memory Tool；实现前必须由用户单独批准本契约和 ADR。
...
因此 Java 不应把同名 Tool 标为逐字节兼容。提议仅增加默认关闭的 `recall_memory` **受限安全替代**：在当前 Chat
...
管理服务当成 Tool Scope Resolver。若本契约获实现授权，`recall_memory` 必须仅由当前 `ChatService` Turn 的私有
...
任一条件不满足时零 Tool 注册；`DISABLED` 时不创建新 Store、Embedding Client、后台任务或网络连接。未知/小写 Mode、
...
获批后才可新增 Java-owned `tools/read-only-memory-recall-v1` Fixture，覆盖 Disabled 零 I/O、严格模式/Schema、
