...
1. Java-owned Fixture 已固定 44 个 Case：原有不透明 Ref、状态优先级、Capsule AAD/篡改、引用替换、未知密钥、v1→v2 迁移及原子 Store 外，新增已批准的一次性消费、未批准拒绝、到期优先、重复/并发 Reservation 非执行权、Reservation 失败回滚、五项 Ledger 终态、两项 Session 条件提交及 O6 的 Anchor Model/初始 SQLite 原子写入 Case。重启、Anchor 恢复提交和实际 Tool Invocation Case 仍留在后续 Capability。
