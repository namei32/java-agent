...
- 已冻结并完成初始无执行安全边界：R11 B2b 的[待审批 Tool Operation、参数胶囊与恢复安全契约](contracts/pending-tool-operation.md)和[ADR-0018](adr/0018-use-single-transaction-pending-operation-store.md)。18 场景 Fixture 已验证不透明引用、状态优先级、AES-256-GCM 胶囊绑定及篡改/替换/未知密钥的失败关闭；单一隔离事务 Store、条件 Conversation 提交及恢复仍未实现，且未接入 Chat 或 Tool。
...
R11 已完成 B1 的[Tool Catalog 与 Capability 治理契约](contracts/tool-catalog-capability-governance.md)和 B2a 的[本地审批收件箱契约](contracts/tool-approval-inbox.md)、[ADR-0017](adr/0017-isolate-durable-approval-inbox.md)、[设计](specs/2026-07-18-r11-approval-inbox-design.md)及[实施计划](plans/2026-07-18-r11-approval-inbox-implementation.md)。B2b 已冻结[Pending Operation 契约](contracts/pending-tool-operation.md)、[ADR-0018](adr/0018-use-single-transaction-pending-operation-store.md)、[设计](specs/2026-07-18-r11-pending-operation-design.md)及[实施计划](plans/2026-07-18-r11-pending-operation-implementation.md)。Catalog 只在一个 Turn 内减少 Schema 暴露；Inbox 的决定也不提供恢复或副作用执行权限。
