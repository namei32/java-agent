...
- 已完成并验证：R11 B1 已冻结 Tool Catalog Contract/ADR/Fixture，并实现 Turn-scoped `tool_search`、常驻/延迟 Schema、CJK/精确检索、同批隔离和 Bootstrap 的静态只读 MCP deferred 投影；R11 的人类审批、Durable Ledger 与真实副作用仍未实现。
...
R11 的当前 B1 边界见[Tool Catalog 与 Capability 治理契约](contracts/tool-catalog-capability-governance.md)、[ADR-0016](adr/0016-use-turn-scoped-tool-catalog.md)、[设计](specs/2026-07-18-r11-tool-catalog-design.md)和[实施计划](plans/2026-07-18-r11-tool-catalog-implementation.md)。Catalog 只在一个 Turn 内减少 Schema 暴露；它不提供审批或任何副作用执行权限。
