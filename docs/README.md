...
- 已完成全量差距审计，并冻结 R12-S1 的[只读 Skill Catalog 契约](contracts/read-only-skill-catalog.md)、[ADR-0020](adr/0020-use-project-owned-read-only-skill-catalog.md)、[设计](specs/2026-07-19-r12-read-only-skill-catalog-design.md)与[实施计划](plans/2026-07-19-r12-read-only-skill-catalog-implementation.md)。它默认 `DISABLED`，不执行 Skill、Tool、脚本或网络。
...
R12-S1 已先冻结[只读 Skill Catalog](contracts/read-only-skill-catalog.md)的安全边界：仅 `READ_ONLY`、受限 Java Roots、无路径 Prompt 投影、依赖存在性检查与 always Skill 注入；实现前不扫描任何 Root，且不执行 Skill/脚本。
