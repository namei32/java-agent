...
- 已冻结并开始实施：R7 受信 Java `ServiceLoader` SPI、隔离 stdio Plugin Bridge 与观察型 Hook；R8 本地 SQLite 租约 Scheduler、受限 Proactive/Drift/Subagent；R9 sandbox 演练、备份、差异和回退门禁。三阶段默认均关闭。
...
R7–R9 已在新的隔离分支上按用户授权开始：R7 的[插件与扩展运行时契约](contracts/plugin-extension-runtime.md)、R8 的[主动运行时契约](contracts/proactive-runtime.md)和 R9 的[生产切换契约](contracts/production-cutover.md)先固定默认关闭、无真实数据/网络/部署的边界；每个阶段在前序门禁通过后才进入下一阶段。真实生产切换仍需独立执行授权。
