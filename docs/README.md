...
- 已冻结并完成初始无执行安全边界：R11 B2b 的[待审批 Tool Operation、参数胶囊与恢复安全契约](contracts/pending-tool-operation.md)和[ADR-0018](adr/0018-use-single-transaction-pending-operation-store.md)。21 场景 Fixture 已验证不透明引用、状态优先级、AES-256-GCM 胶囊绑定及篡改/替换/未知密钥的失败关闭，以及 SQLite v1→v2 迁移和 Inbox+密文胶囊原子保存；条件 Conversation 提交、Reservation 消费及恢复仍未实现，且未接入 Chat 或 Tool。
