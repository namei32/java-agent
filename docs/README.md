...
- 已完成当前授权范围：R6.5 默认关闭、仅 Loopback 的认证控制面后端 G0–G10，包括 48 Case Java-owned Fixture、活动 Turn Registry、独立有界 Event Hub、Telegram Volatile/SQLite Reliable 接入、进程内 Bearer Session、安全状态/取消 API、future-only SSE 和故障矩阵；Draft PR #9 首轮远程三套 CI 全绿，本地高风险 Review 修复及默认 499、`failure` 149、`compat` 154 项阶段门禁也已全绿，等待推送/Ready 授权。
