...
- 已形成待审批草案：R6.4 独立 Java SQLite 渠道账本、Inbox/Turn Claim、事务 Outbox、Receipt、`UNKNOWN`、恢复、清理和回退边界；新 Schema 与生产实现尚未开始。
...
R6.4 的待审批边界见 [渠道可靠投递、幂等与恢复契约](contracts/channel-reliable-delivery.md)、[ADR-0010（提议）](adr/0010-use-dedicated-sqlite-channel-ledger.md)、[设计草案](specs/2026-07-16-channel-reliable-delivery-design.md)、[实施计划](plans/2026-07-16-channel-reliable-delivery-implementation.md)和[备份/回退手册草案](runbooks/channel-ledger-backup-rollback.md)。当前只完成决策材料，不得据此创建 `channel-ledger.db`、自动重跑 Turn、重发 `UNKNOWN` 或访问真实 Telegram。
