...
- 已冻结、尚未实现：R14-P6 的[获批 Java-native NOTE 写入演练 Contract](contracts/r14-p6-approved-proactive-memory-note-write-rehearsal.md)只允许 `@TempDir` 中的 Fake Embedding/Java SQLite DML；它明确固定 Scope、测试生命周期保留、Approval、Mutation Ledger、回退和零自动重放，不增加任何生产入口。
