...
- 设计中：R4.1 只读 Markdown Profile、Context Frame 与 Retrieval Port 纵向切片；当前 Contract/Spec/Plan 待批准，尚未读取真实 Workspace 或编写生产实现。
- 下一里程碑：沿 Python 重写主线完成 R4 只读上下文与记忆，再进入 MCP、渠道、插件和主动运行时；Approval Channel、生产 Durable Ledger 与真实副作用工具暂时冻结。
...
当前 Golden 资产从 [Python/Java Golden Test 夹具规范](contracts/golden-test-fixtures.md)进入，包含配置、历史、Prompt、SQLite、错误映射、Tool Loop、Tool Runtime 安全和 Approval/Side Effect 基准。
...
R4.1 的待批准边界见 [只读上下文与记忆兼容契约](contracts/read-only-context-memory.md)、[设计](specs/2026-07-14-read-only-context-memory-design.md)和[实施计划](plans/2026-07-14-read-only-context-memory-implementation.md)。
