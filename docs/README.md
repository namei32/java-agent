...
- 已完成并验证：R11 B1 Tool Catalog 与 B2a Local Approval Inbox Foundation。B2a 提供默认关闭、独立 SQLite、28 场景 Fixture、Loopback Operator 的安全列表/一次性决定、过期/容量/CAS 与零执行边界；它不恢复 Turn，不调用 Tool。
...
R11 已完成 B1 的[Tool Catalog 与 Capability 治理契约](contracts/tool-catalog-capability-governance.md)和 B2a 的[本地审批收件箱契约](contracts/tool-approval-inbox.md)、[ADR-0017](adr/0017-isolate-durable-approval-inbox.md)、[设计](specs/2026-07-18-r11-approval-inbox-design.md)及[实施计划](plans/2026-07-18-r11-approval-inbox-implementation.md)。Catalog 只在一个 Turn 内减少 Schema 暴露；Inbox 的决定也不提供恢复或副作用执行权限。
