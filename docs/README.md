...
- 下一里程碑：审批、副作用、幂等和沙箱 Contract/Spec/Plan 已批准，下一步按计划实现生产 Deny All 的 Framework；不能直接跳到真实工具或完整工具生态扩展。
...
R3.2 的批准边界从 [Tool 审批、副作用、幂等与沙箱安全契约](contracts/tool-approval-side-effect-safety.md)进入，实现依据见 [Tool Approval Framework 设计](specs/2026-07-14-tool-approval-framework-design.md)和[实施计划](plans/2026-07-14-tool-approval-framework-implementation.md)。批准只覆盖生产 Deny All 的 Framework，不授权任何真实副作用工具。
