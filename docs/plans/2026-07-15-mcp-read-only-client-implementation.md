...
- 状态：已实现并验证（Task M0–M8）
- 日期：2026-07-15
- 阶段：R5.1
- 基线：R4.2 Task J1–J12 已完成并合并到 `main`，合并提交 `77cda94`
- 批准记录：2026-07-15，用户要求先合并并推送本地 Java 主线，随后开始实现本计划
- 实施授权：按 Task M0–M8 实现受控 Java Reference Server 纵向切片；不授权真实 MCP Server、网络、Secret 或副作用工具
...
M0 源码 Spike 已确认 SDK `2.0.0` 的 Client/Schema/Jackson 3 可用，但内置 Stdio Transport 无法满足环境清空、反序列化前 Wire 上限和 Wire Cancellation。ADR-0006 因此固定“官方 Client + Adapter 自有有界可取消 Transport”；M5 必须以 Wire 测试证明该扩展，证明失败则暂停并保持 `DISABLED`。
...
- Bootstrap 通过独立 `McpProperties` 绑定全部 R5.1 上限；`DISABLED` 直接返回空 Runtime，不读取配置或启动进程。
- `STATIC_READ_ONLY` 在读取配置前验证全局 Tool Runtime、MCP Request/Tool Timeout 和 Connect/Model Timeout 关系。
- MCP Tool 与 `current_time` 合并为不可变只读快照；Bootstrap 边界再次拒绝任何非 `READ_ONLY` Tool，并由 Spring Destroy Hook 幂等关闭 Runtime。
- `application.yml` 与 `.env.example` 保持 MCP `DISABLED`，SDK 包日志为 `OFF`；生产没有 SDK 类型、动态管理 Tool、HTTP Transport 或副作用 MCP Tool。

TDD/Contract 证据：

| Task | RED/Contract 证据 | GREEN 结果 |
| --- | --- | --- |
| M0 | SDK Spike 证明内置 stdio 在环境、Wire、Cancellation 与关闭上不满足契约 | ADR-0006 固定官方 Client + Adapter 自有 Transport |
| M1 | Java-owned Fixture/Manifest 先冻结 22 个配置、命名、Schema、结果与生命周期 Case | `McpGoldenManifestTest` 验证内容与 Hash |
| M2 | 测试先要求尚不存在的模块与公开 Runtime 边界 | SDK/Reactor 只留在 `adapter-mcp`，Kernel/Application 架构测试通过 |
| M3 | 配置、路径、Shell、环境和上限 Case 在 Loader 前先固定 | `McpConfigurationTest` 7 项通过，损坏配置在首个进程前失败 |
| M4 | 名称/Unicode/碰撞与 Schema 安全 Case 先固定；Unicode 期望曾形成有效失败并修正 | Name Mapper 2 项、Projector 5 项通过 |
| M5 | 缺失 Client/Transport/取消关联使 stdio 与 Wire 测试先失败 | Stdio 3 项、Cancellation 1 项通过，覆盖 EOF/TERM/KILL 与准确 Request ID |
| M6 | 缺失 MCP Tool Adapter/Runtime 使 Tool/Chat 闭环先失败 | Adapter 2 项、Chat IT 1 项通过，SQLite 只提交最终 User/Assistant |
| M7 | 损坏 JSON、噪声、错误 ID、断线、Stale 与竞态 Case 先失败 | `McpFailureTest` 8 项进入 `failure` Profile 并通过 |
| M8 | Bootstrap 测试先因缺少 `McpProperties`/`configuredTools` 编译失败，模板测试随后因缺少 MCP 默认项失败 | Bootstrap 8 项通过，完整 Spring 默认上下文保持 MCP 零启动 |

提交证据：`32a8267`、`0c8d751`、`f91ed8c`、`8cd3e6f`、`7db5f60`、`2c9581e`、`4a69e80`、`6a1d55e`、`80963a6`、`a5d7264`。

最终门禁（2026-07-15）：

- `spotless:check` 与干净默认 Reactor 通过：284 项（270 单元、14 集成）。
- `failure` 通过：63 项（62 单元、1 集成）。
- `compat` 通过：323 项（308 单元、15 集成）。
- 三套门禁均为 0 Failure、0 Error、0 Skipped；`adapter-mcp` 默认覆盖 27 个单元和 5 个集成测试。
- Kernel Dependency Tree 只有 Test Scope 的 JUnit、AssertJ、Jackson，无生产依赖；MCP Adapter 生产依赖为 Kernel、官方 SDK `2.0.0` 及其隔离的 Jackson/Reactor/Validator 传递依赖，Application/SQLite 只为 Test Scope。
- Kernel/Application 的 MCP SDK、Reactor、Spring MCP/JSON-RPC 扫描零命中；生产 Python/Shell/HTTP Transport/动态管理/副作用能力零命中。唯一 `/bin/sh` 字符串位于“拒绝 Shell”配置测试，未执行。
- 常见真实 Secret 格式、被跟踪 `.env`/`config.toml`/数据库/日志扫描零命中；`jps` 未发现存活 `McpJavaReferenceServer`。
- 全过程未运行 Python、真实 MCP Server、真实 Secret、外部网络、真实 Workspace 或真实副作用。
...
- 官方 SDK Client 配合 Adapter 自有 Transport 仍无法把外层取消可靠转换成 MCP Cancellation。
...
- Adapter 自有 Transport 无法在反序列化前实施 Wire 字节上限。
