# R11-B2c 首个副作用 Capability 候选与授权边界

- 状态：候选已审计；**未获逐工具执行授权**，零生产路由、零 Capability、零 Tool 注册
- 日期：2026-07-19
...
当前最小、最能验证 R11-B2c 恢复闭环的候选是 Java 原生记忆的 `forget_memory`：它只允许在精确 Session Scope
中物理删除一个已知 Memory ID。它不访问网络、不启动进程、不读取或写入 Workspace 文件，也不碰已明确弃用的
Python `memory2.db`。

但删除用户的 Java 原生记忆仍是不可逆副作用。它不能因为已有 SQLite Store、Fake Capability 或全局“重写 Python
Agent”目标而被视为已获执行授权；在用户明确批准这个准确的 Tool、风险和边界前，系统必须继续保持生产 Deny All、
无 Pending Recovery 路由、无 Worker、无 `forget_memory` Tool。

## 已提交 Python 基线与候选筛选

| Python Tool | 副作用/依赖 | 当前 Java 基础 | 本轮结论 |
| --- | --- | --- | --- |
| `forget_memory` | 删除一个或多个记忆 | Java 原生记忆已有 Scope 绑定、物理删除和幂等 Mutation Ledger | **推荐首个候选**；先缩为每次一个 opaque ID，需删除授权 |
...
## 拟议 `forget_memory` Capability V1（仅供授权审阅）

1. Tool 名称固定为 `forget_memory`，版本固定为 `java-memory-forget-v1`，风险为 `WRITE`；不能由模型、Plugin、MCP
   或配置字符串动态创建。
...
1. **F1 Contract Fixture（RED）。** 冻结 Capability 名称/版本、严格单 ID 参数、模式组合、脱敏结果、默认零注册、
   Anchor/Approval/Reservation/取消/过期/并发/`UNKNOWN`/`COMMIT_UNREPORTED` 语义。
2. **F2 Capability Kernel（GREEN）。** 引入不可变、静态 Capability Descriptor 与单 ID 删除 Invoker Port；无 JDBC、
   Spring 或 Tool Registry 依赖。
...
4. **F4 Java Memory Adapter（GREEN）。** 将精确 Scope 的 `MemoryDeleteService` 包装为 Invoker，验证 Capsule 绑定与
   安全 Result；不为测试或运行时创建 Python Memory 文件。
5. **F5 纵向失败矩阵（GREEN）。** 用真实临时 SQLite 验证单获胜者、撤销/过期/新 Turn 零删除、Commit Failure 零重放
   以及关闭/中断；随后运行默认、`failure`、`compat` 三套完整门禁。
6. **F6 文档与审查。** 回写 Contract、Capability 审计、Roadmap、矩阵和运行手册；仍不进行真实数据 Smoke，除非
   另有运行授权。

## 需要的明确决策

继续实现 F1–F6 前，需要用户确认：**批准 `forget_memory` V1 作为首个副作用 Capability；范围仅限 Java 原生
记忆库中当前 Session 的单 ID 物理删除，默认关闭、Loopback 认证、人工审批与本地 Fake 测试，不启用真实数据、
网络、Workspace、Telegram 或 Python 记忆访问。**
