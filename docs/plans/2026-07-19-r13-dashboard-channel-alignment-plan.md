...
当前用户冻结继续生效：**不实现远程访问、真实 Telegram、CLI+Web、前端或 Web API 扩展**。因此本文只安排后续
顺序和验收边界；不会以本计划授权网络、真实数据、渠道消息投递、持久化写入或前端文件。
...
在现有 Operator Session 和 Loopback Filter 后，仅增加活动 Turn/渠道健康的有界投影。默认关闭；不读取历史正文、
不访问真实 Telegram、无前端、无写入。用 Fake Runtime、固定 Clock 和临时 SQLite 验证认证、Scope、容量、取消和关闭。

### C2：受限历史浏览（GREEN）

在单独批准后，增加按不透明 Ref 查询的 Session/Message 只读投影。必须定义数据保留、角色/正文脱敏、页大小、
全文检索是否允许，以及 Ref 过期/撤销。不得把 R11-B4 的模型证据 Tool 或 HTTP Memory API 复用为 Dashboard 越权读取。
