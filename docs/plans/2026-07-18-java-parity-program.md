...
| R14 | Peer、真实 Proactive/Drift 与 Memory 自动化 | `agent/peer_agent/*`、`agent/core/proactive_*`、`core/memory/*` | 身份/信任、外部源、写入许可、审计、预算、恢复与回退契约 | P0 已完成：28 Case Java-owned Fixture、Scheduler/租约状态表、本地 Fake Source 净化、待审批 Delivery 投影、`NONE` Memory Mutation 与 `LOCAL_FAKE` Peer 值对象均由 Contract 消费；P1–P5 仍未实现。R8/P0 不是 Python 自动记忆、外部主动源、Peer 进程/A2A 或真实投递对齐 |
