...
| --- | --- | --- | --- | --- |
| R10 | Prompt、Persona、时间、预算与 Provider 协议 | `agent/context.py`、`agent/core/prompt_block.py`、`agent/persona.py`、`prompts/agent.py`、`agent/provider.py` | Section/Frame/预算/裁剪有 Fixture，`AKASHIC_CORE` 可离线验收；Provider 的失败/Options/thinking/cache 分层对齐 | Prompt/Persona 与 Provider P0 脱敏失败分类已实现并验证；P1 Options 已审计、等待运行语义选择，P2–P4 未开始 |
...
| R12 | Skills、MCP 扩展、Plugin 与受限 Memory Tool | `agent/skills.py`、`agent/mcp/*`、`agent/plugins/*`、`agent/tools/recall_memory.py` | 受信 Skill Catalog/执行边界、Python MCP Tool Client、Plugin 生命周期兼容、受限当前 Scope 召回 | S1、S3、S4 已通过三套门禁；S2 是 Java-owned Assets 目录。S5 已实现默认关闭的 `recall_memory` 安全替代，不声称 Python 检索等价；Skill 执行、远程 MCP、可变 Plugin 与任何记忆写入仍未开始 |
