...
| R13 | 多渠道、Dashboard 与控制面 | `infra/channels/*`、`bootstrap/dashboard_api.py` | 频道 Contract、真实渠道验收、前端/控制面完整 API 与安全边界 | R13-C0/C1 已完成：20 Case Contract 已落为默认关闭的本机 `GET /api/v1/control/index`，仅经现有 Loopback/Bearer 投影活动 Turn/渠道健康与一次性 opaque cursor。C2–C5 未开始；现有控制面和这条索引都不是 Python Dashboard、IPC/QQ/Plugin Channel 或前端对齐，远程、真实 Telegram、CLI+Web 与前端继续冻结 |
