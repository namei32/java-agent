...
| R11 | Tool Catalog、审批与逐工具 Capability | `agent/tool_bundles.py`、`agent/tools/*`、`agent/tool_runtime.py` | 可用人类审批、Durable Ledger、每个副作用 Tool 的幂等/UNKNOWN/沙箱 Contract | 实施中：B1 Tool Catalog 与 B2a 本地 Approval Inbox 已验证；B2b Pending Operation/参数胶囊/条件提交 Contract 已冻结，尚未实现；Ledger 与真实 Capability 未开始 |
