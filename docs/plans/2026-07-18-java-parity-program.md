...
| R12 | Skills、MCP 扩展与 Plugin 能力 | `agent/skills.py`、`agent/mcp/*`、`agent/plugins/*` | 受信 Skill Catalog/执行边界、MCP Resources/Prompts/Streamable HTTP、Plugin 生命周期兼容 | R12-S1 Skill Catalog、S2 MCP Assets 目录和 S3 API v2 只读 Lifecycle Tap 均已实现并通过三套门禁；按需正文/执行、远程 MCP 与可变 Plugin 生命周期仍未开始 |
