...
| --- | --- | --- | --- | --- |
| R10 | Prompt、Persona、时间、预算与 Provider 协议 | `agent/context.py`、`agent/core/prompt_block.py`、`agent/persona.py`、`prompts/agent.py`、`agent/provider.py` | Section/Frame/预算/裁剪有 Fixture，`AKASHIC_CORE` 可离线验收；Provider 的失败/Options/thinking/cache 分层对齐 | Prompt/Persona、Provider P0 脱敏失败分类和 P3 默认关闭、Tool 前非流式上下文恢复均已实现并验证；P1 Options 已审计、等待运行语义选择，P2/P4 未开始 |
