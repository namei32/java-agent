...
| --- | --- | --- | --- | --- |
| R10 | Prompt、Persona、时间、预算与 Provider 协议 | `agent/context.py`、`agent/core/prompt_block.py`、`agent/persona.py`、`prompts/agent.py`、`agent/provider.py` | Section/Frame/预算/裁剪有 Fixture，`AKASHIC_CORE` 可离线验收；Provider 的失败/Options/thinking/cache 分层对齐 | Prompt/Persona、Provider P0 脱敏失败分类、P1 默认关闭的无 Tool 文本受信 Options、P3 默认关闭的 Tool 前非流式上下文恢复，以及 P4 提交后匿名 cache prompt/hit 聚合均已实现并验证；P2 未开始 |
