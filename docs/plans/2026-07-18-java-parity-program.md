...
| R11 | Tool Catalog、审批与逐工具 Capability | `agent/tool_bundles.py`、`agent/tools/*`、`agent/tool_runtime.py` | 可用人类审批、Durable Ledger、每个副作用 Tool 的幂等/UNKNOWN/沙箱 Contract | 实施中：B1 Tool Catalog 与 B2a 本地 Approval Inbox 已验证；B2b Pending Operation 的无执行状态机、AES-GCM 胶囊与 Fixture 已验证，隔离 Store、条件提交、Ledger 与真实 Capability 未开始 |
