...
| 记忆与上下文 | Java Native Memory、当前 Scope 检索、Context 注入、只读 `recall_memory`、已验证的 `forget_memory` 离线链 | 自动写回、Optimizer、全局管理与真实 Embedding | 等待 C3-M0 选择新的管理入口；其他 Memory 写入逐项选择 |
| Tool 与审批安全 | Catalog、Deferred Tool、Approval Inbox、Capsule、Reservation、Ledger、Anchor、首个 `forget_memory` 链 | 其他写 Tool、真实审批操作与 Sandbox | C3-F2/F3 已验证；不自动扩展为新写 API，M1 需先取得目标 Ref/请求/数据权限决定 |
