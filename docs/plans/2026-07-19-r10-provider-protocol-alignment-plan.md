...
- 状态：P0、P1、P3、P4 与受限即时 P2 已完成并离线验证；P2b 未实现
- 日期：2026-07-19
...
#### P2b：DeepSeek reasoning 历史与空占位符（未实现，需数据保留授权）

审计 Python `DeepSeekStrategy.prepare_request` 与 `Session.get_history` 后确认：Python 会为 thinking 请求的历史 assistant
消息补 `reasoning_content: ""`，且将非空 reasoning 保存在 Session/Tool Chain 中。Java P2 有意不持久化，并受 Spring
AI 2.0 的“仅非空 metadata 才发 wire field”限制，因此不能声称这一点已经对齐。P2b 若要实现，必须先冻结 reasoning
的 Session Schema、保留/删除、导出/日志隔离、跨 Provider 清理和空字段请求扩展 Contract；未经这项批准不得扩大 P2 的
内存边界。
...
- P0、P1、P3、P4 与受限即时 P2 均已完成离线实现与门禁；P2b 的跨 Turn reasoning 历史/空占位符仍未实现。P1/P2/P4 的
  受限 Options、单轮 reasoning 和匿名观测均不构成 Python Dashboard/诊断 UI、任意 Provider fields 或真实 Provider
  Smoke 授权。真实 Provider Smoke 仍为独立运行授权。
