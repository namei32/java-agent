...
- 状态：后端 G0–G10 与本地高风险 Review 修复已完成并通过阶段门禁；Draft PR #9 等待推送/Ready 授权
- 日期：2026-07-17
...
G5–G9 阶段门禁证据（2026-07-18）：新增默认 30、`failure` 8、`compat` 48 项后，三组实际为
491、146、154 项，仍按 Tag 互斥且全部零失败。`spotless:check`、默认 `clean verify`、`failure`
和 `compat` 的本地耗时分别为 0.599、40.471、44.300、26.554 秒。所有自动化仅使用本地
Stub/Fake、受控 Java Reference MCP Server、临时数据库和 Loopback；未读取真实 Telegram Token、
访问外网、启动前端或触碰用户 Workspace。
...
- Volatile Turn 只在即将调用 Application 时注册，同一 Buffer 负责真实取消；Worker/Producer
  启动失败发生在执行边界前，因此零注册、零 Tombstone。
...
- Telegram 与 Dashboard 都复用现有权威取消源；本 Task 验证接线、注册边界和 Durable Commit 边界，
  不重复建立完整 First Writer 并发矩阵。
...
RED/GREEN 证据（2026-07-18）：Review 发现 Volatile 路径在 Turn Worker 启动前注册控制面，违背
“即将调用 `MessageTurnService`”的执行边界。扩展既有 Worker/Producer 启动失败参数化测试后，
聚焦命令稳定执行 2 个场景并因两者都留下 `SOURCE_ENDED` Tombstone 而失败。将注册移动到
`produce()` 内、紧邻 `turns.process(...)` 前后，同一命令 2 个场景全部通过，启动失败现在零注册、
零 Tombstone。

G4 聚焦门禁一次执行 `ReliableInboundCoordinatorTest` 10 项及 Bootstrap 三个目标类 16 项，共
26 项全部通过，Reactor 用时 4.406 秒。Volatile/SQLite Reliable 都复用已有取消源；Reliable
注册发生在 Claim `RUNNING` 后，终态只在 Durable Commit 后投影，Commit 失败不产生假终态；
Registry 饱和、Observer 失败和 `EXECUTION_UNKNOWN` 保持主链路隔离。完整 First Writer 并发矩阵
明确单点归属 G9，不在 G4 重复。`git diff --check` 通过，未使用真实 Telegram、网络、Token、
用户工作区或前端。
...
RED/GREEN 证据（2026-07-18）：先加入 Properties、Configuration、Disabled Bootstrap 和
Loopback Binding 四类测试，聚焦命令在测试编译阶段因 `ControlPlaneMode`、Properties、Runtime
和 Configuration 缺失而失败。最小实现后同一命令执行 9 项全部通过：模式值大小写严格，11 项
预算按 Contract 默认值和硬边界绑定；只有 Servlet + `LOOPBACK` + 字面 `127.0.0.1`/`::1`
创建单例 Runtime，Wildcard、远端、Hostname、空地址及 Non-Web CLI 全部在 Runtime 前失败。

Runtime 同时是生产 `ActiveTurnObserver`，内部只持有有界 Registry/Event Hub，不创建 Worker；
Telegram 普通/可靠 Spring Bean 都从容器获得该 Observer，Disabled 时回落 No-op。默认 Disabled
不生成 Turn Ref、不创建 Runtime/Registry/Hub/Subscriber 或文件。G5 聚焦命令 9 项通过，随后只补跑
受签名接线影响的 `TelegramBootstrapTest` 与 `TelegramReliabilityBootstrapTest` 13 项，也全部通过。
...
RED/GREEN 证据（2026-07-18）：先加入 SSE Controller、纵向集成和 Failure IT，聚焦命令在
测试编译阶段因 Writer/Factory/Controller 缺失而失败。最小实现后最终聚焦 `verify` 执行 4 项
单元/状态测试和 2 项 SSE 集成/故障测试全部通过。建立流前统一验证 `Accept`、Replay Header、
canonical Ref、Turn 状态与 Subscriber 容量，仍可返回固定 JSON Error；Header 建立后只写 UTF-8
SSE，不尝试以 JSON 覆盖流内故障。

每个订阅先写独立 `id: 0` 的 future-only Opening，随后按订阅独立序号写经过 Kernel 安全投影的
Message；JSON 由 ObjectMapper 编码，换行正文被转义，原始 Turn/Session/Route 不进入帧。Terminal
发送后正常结束；Heartbeat、Session 到期/注销、Writer I/O、生命期和 Shutdown 都通过有界 Poll/
Hub 关闭释放订阅，不创建每事件任务，也不改变 Turn 取消源。Event Hub 新增真实 Subscriber 数、
最大队列深度、容量和饱和慢消费者断开计数，Status 不再返回占位零值。Writer 故障测试确认只释放
自身订阅且不取消 Turn。Spotless 与 `git diff --check` 通过；严格全门禁继续留给 G9 后统一执行。
...
- 完整 First Writer 矩阵：Dashboard Cancel 与 Telegram Control、Disconnect、Backpressure、
  Shutdown、Terminal；这一矩阵由 G9 单点负责，G4 不跨层重复。
...
实现证据（2026-07-18）：`LoopbackControlPlaneGoldenTest` 动态执行全部 48 Case。简单 Case 按
Mode/Security、Session、Status/Turn、Cancellation、SSE 和 Shutdown/Failure 分组经过生产
Validator/Store/Registry/Projection/Audit 边界；HTTP、并发、SQLite、阻塞 Writer 和 Spring 生命周期
Case 显式声明唯一聚焦测试所有者，并校验源码类和方法存在，不在 Compat 中复制故障矩阵。
`ControlPlaneConcurrencyFailureIT` 固定 Dashboard 重复取消以及与 Disconnect、
Backpressure、Shutdown 的 First Writer 竞争；Application `failure` 矩阵继续单点覆盖 Terminal 和
Event Hub 竞争，G4 不重复跨层并发矩阵。新增 Security 1 项、并发 4 项、两种销毁顺序 2 项均通过；
既有可靠投递故障测试继续覆盖 Running Claim、Terminal Commit Failure 与 `EXECUTION_UNKNOWN`。
聚焦 `failure` 命令执行目标 Failsafe 6 项，聚焦 `compat` 执行 48 项，随后四项严格阶段门禁全部通过。
提交为 `71def8e`。

## 13. Task G10：后端最终门禁、Review、PR 与合并

状态：首轮远程三套 CI 已通过；本地高风险 Review 修复和阶段门禁已完成，尚未推送修复、转 Ready 或合并。

先更新 Contract/Spec/ADR/计划状态、Manifest Hash、文档导航、R6 总计划、Roadmap、重写指南、能力矩阵和配置说明。记录实际测试数，不预填。

G9 后已经统一执行一次完整离线门禁；G10 不重复本地测试矩阵，只补依赖、Diff 和发布审计：
...
本地 Review 证据（2026-07-18）：Kernel 生产依赖为空，Application 生产依赖只有 Kernel；Markdown
相对链接、Spotless、完整分支 Diff、数据库产物和原始工作树隔离检查均通过。安全复核发现
`ControlSessionResponse` 的 record 默认字符串会包含一次性 Bearer，先补聚焦 RED，再覆盖为
`accessToken=<redacted>`；Session/Security 聚焦 8 项 GREEN，修复提交为 `443549b`。JSON 首次创建
Session 时返回 Token 的协议保持不变。

远程证据（2026-07-18）：Draft PR #9 首轮 Run `29631194248` 的 `failure`/`compat` 通过，默认 Job
暴露 `ReliableInboundCoordinatorTest` 在取消唤醒后与 `SOURCE_ENDED` 注销竞争，慢 Runner 可能先观察
Tombstone。修复仅在测试中增加允许 Worker 收尾的确定性 Latch，不改生产 First Writer 语义且无
Sleep；目标测试本地连续 3 次通过，提交 `e3dc7a8`。第二轮 Run `29631304643` 的默认、`failure`、
`compat` 分别在 1 分 47 秒、1 分 17 秒、50 秒通过。PR 继续保持 Draft；真实 Telegram、远程访问、
CLI+Web、前端、Ready 和合并均未执行。

本地高风险 Review 修复（2026-07-18，尚未推送）：

- 启动门禁拒绝非 `NONE` 的 Forwarded Header 翻译，防止容器先改写 Loopback 判断输入。
- 慢消费者和 Terminal 只退出 Turn Fan-out；全局 Subscriber 预约保留到 Servlet `finally`，关闭中的请求不能绕过容量上限。
- 取消、慢消费者、Writer/生命周期关闭和 Shutdown Timeout 使用稳定安全审计；默认 Sink 只记录结构化白名单字段。
- 新增 Stream Tracker 与 Shutdown Coordinator，用一个共享 Deadline 等待真实 Servlet 流；超时不取消 Turn，只记录剩余流数。
- 48 Case Golden 区分直接生产边界与唯一聚焦测试所有权，补齐 Status、Session 容量和关闭失败语义，避免重复并发矩阵。

这些修复的聚焦 RED/GREEN 已通过。恢复后的本地提交 `852294a` 又通过 Spotless、Diff、默认、
`failure` 和 `compat` 阶段门禁：三组分别执行 499、149、154 项，全部 0 Failure、0 Error、0
Skipped；`failure` 与 `compat` 墙钟分别为 37.67、14.27 秒。当前停在推送与 Ready 授权点。
