...
- 分布式顺序保证、远程部署、认证和多租户。
- Flyway、JPA、Hibernate、R2DBC、WebFlux、Kafka、Redis、GraalVM、Docker 镜像和 Maven Central 发布。
...
开始创建工程骨架时，应先检查当时可用的稳定版本，再在实施计划中记录准确的补丁版本；所选版本必须位于已批准的次版本线内。
...
1. 提交本设计，并由用户复核书面文件。
2. 使用 `writing-plans` 创建逐文件的实施计划。
3. 在隔离的功能分支或 Git Worktree 中工作。
4. 使用 TDD 实现每个行为。
5. 对兼容性相关工作进行独立的规格审查和质量审查。
6. 声称完成前执行全新的目标验证和完整验证。
7. 使用 `finishing-a-development-branch` 完成集成。

书面规格通过用户复核且实施计划存在之前，禁止创建生产工程骨架或编写生产代码。
