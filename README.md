...
Java 原生语义记忆不读取、迁移或删除 Python `memory2.db`，也不会从普通聊天自动提取记忆。模板和部署默认保持 `AGENT_MEMORY_MODE=DISABLED`；当前不包含 Optimizer、自动记忆提取、Memory Tool、远程 Vector Store 或真实用户数据迁移。R8 Scheduler 仅在显式 `LOCAL_SQLITE` 下运行本地 allowlist/NoOp 演练，不调用 Provider、Telegram 或 Workspace 写入。
...
R12-S1–S3 已完成默认关闭的只读扩展切片：Skill Catalog、MCP Resources/Prompts 元数据目录，以及 API v2
`LIFECYCLE_TAP`。后者仅将受支持的 Java 生命周期投影为 Phase、安全 Hash 与 Outcome；外部 stdio Plugin 必须显式
选择 `api-version: 2`。三项均不读取未批准正文、不执行 Skill、不导入 Python、不提供可变 Plugin Gate，也不启用真实网络或外部 Plugin。
