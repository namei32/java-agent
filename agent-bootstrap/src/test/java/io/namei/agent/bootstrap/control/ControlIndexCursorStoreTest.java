...
class ControlIndexCursorStoreTest {
  private static final String ACTOR = "AAAAAAAAAAAAAAAAAAAAAA";

  @Test
  void keepsOnlyOpaqueActorBoundShortLivedOneTimePages() {
    MutableClock clock = new MutableClock(Instant.parse("2026-07-19T00:00:00Z"));
    ControlIndexCursorStore store =
        new ControlIndexCursorStore(clock, sequentialRandom(), Duration.ofMinutes(1), 2);
    List<ControlIndexResponse.Turn> remaining =
        List.of(new ControlIndexResponse.Turn(reference(1).value(), "telegram", "ACTIVE", 3L));
  ...
  @Test
  void evictsTheOldestPageWhenItsBoundedCapacityIsReached() {
    ControlIndexCursorStore store =
        new ControlIndexCursorStore(
            Clock.fixed(Instant.parse("2026-07-19T00:00:00Z"), ZoneOffset.UTC),
            sequentialRandom(),
            Duration.ofMinutes(1),
            1);
    List<ControlIndexResponse.Turn> page =
        List.of(new ControlIndexResponse.Turn(reference(1).value(), "cli", "ACTIVE", null));

    String first = store.issue(ACTOR, page);
    String second = store.issue(ACTOR, page);

    assertThat(store.take(first, ACTOR)).isEmpty();
    assertThat(store.take(second, ACTOR)).contains(page);
  }
